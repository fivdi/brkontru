...
function Led(pin, options) {
  ...
  dto.install(config, function (err) {
    ...
    findTriggerFile(this.name, function (err, device) {
      if (err) return this.emit('error', err);
...
function findTriggerFile(name, cb) {
  function find() {
    var PATTERN = LED_ROOT_PATH + name + '/trigger';

    glob(PATTERN, null, function (err, matches) {
      if (err) return cb(err);
      if (matches.length === 0) return setImmediate(find); // TODO - Stop looping at some point!
      if (matches.length !== 1) {
        return cb(new Error('No trigger file matching \'' + PATTERN + '\' found'));
      }
      cb(null, matches[0]);
    });
  }
