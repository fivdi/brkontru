...
Led.prototype.brightness = function(val) {
  writeNumber(this.brightnessFd, val, MAX_BRIGHTNESS, 0);
};
...
Led.prototype.delay = function(delayOn, delayOff) {
  ...
  writeNumber(this.delayOnFd, delayOn, NUMBER_POOL_MAX, 500);
  writeNumber(this.delayOffFd, delayOff, NUMBER_POOL_MAX, 500);
};
...
function writeNumber(fd, num, maxNum, defaultNum) {
  var numBuf;

  if (fd != -1) {
    if (num >= 0 && num <= maxNum) {
      numBuf = NUMBER_POOL[num];
    } else {
      numBuf = NUMBER_POOL[defaultNum];
    }
