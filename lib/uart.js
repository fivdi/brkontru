...
function Uart(uartPath, options) {
  ...
  if (typeof uartPath === 'string') {
    // wait for /dev/ttyO<number> (uartPath)
    createStreams(this, uartPath, options);
  } else {
    // add uart to device tree
    // wait for /dev/ttyO<number>
    createStreams(this, uartPath, options);
  }
}
...
function createStreams(uart, uartPath, options) {
  uart._rxfd = -1;
  uart._txfd = -1;

  uart._rxstream = fs.createReadStream(uartPath, {
    highWaterMark: options.highWaterMark,
    encoding: options.encoding,
  });
  uart._txstream = fs.createWriteStream(uartPath, {
    highWaterMark: options.highWaterMark,
    encoding: options.encoding,
    flags: 'r+'
  });
