...
function Pwm(pin, options) {
  ...
  installPwmSubSystem = function () {
    ...
    } else if (!pwmSubSystems[pin.pwm.subSystem].ready) {
      // If 2 or more Pwm objects in the same pwm sub system are created in
      // quick succession, the pwm sub system which is installed when the 1st
      // Pwm is created may not be ready yet, so wait till it's ready.
      pwmSubSystems[pin.pwm.subSystem].once('ready', installPwmModule);
    } else {
      installPwmModule();
    }
  ...
  installPwmModule = function () {
    ...
    } else if (!pwmModules[pin.pwm.module].ready) {
      // If 2 or more Pwm objects in the same pwm module are created in quick
      // succession, the pwm module which is installed when the 1st Pwm is
      // created may not be ready yet, so wait till it's ready.
      pwmModules[pin.pwm.module].once('ready', installPwm);
    } else {
      installPwm();
    }
